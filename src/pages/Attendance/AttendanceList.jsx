           ...
           <div className="space-y-4">
             {records.map((record) => (
               <div key={record.id} className="flex items-center justify-between border-b border-slate-50 pb-3 last:border-0">
                 <div>
                   <p className="font-medium text-slate-700">{record.batch}</p>
                   <p className="text-xs text-slate-400">{record.date}</p>
                 </div>
                 <div className="text-right">
                   <p className="text-sm font-bold text-emerald-600">{record.present} Present</p>
                   <p className="text-xs text-red-400">{record.absent} Absent</p>
                 </div>
               </div>
             ))}
           </div>
